...
- **Check released versions before bumping / advising on version state**: run `gh release list --limit 5` (or `gh api repos/matthewjenner/Klakr/releases/latest --jq .tag_name`) to see what has actually shipped. Never assume the current `Directory.Build.props` value is unpublished just because you haven't seen it pushed in the current session. If a release for the current version already exists on GitHub, CI will no-op on push and the fix won't ship - you must bump. Applies to this session and any inferences about "the user is on an older version."
...
- **Adjust Keep Awake**: `Services/KeepAwakeService.cs` owns a 5-second poll loop and dispatches per `KeepAwakeMode` (idle-only key send / blind key send / STES / STES-allow-screensaver). Platform helpers in `Platform/Windows/StayAwake.cs` (SetThreadExecutionState) and `Platform/Windows/IdleTime.cs` (GetLastInputInfo). Active hours are a 24-bit mask on `AppSettings.KeepAwakeActiveHoursMask` (bit N = hour N of local day; 0 = always active), matched via `HourMask.Contains` inside `KeepAwakeService.Evaluate`. UI is a 12-column `UniformGrid` of `ToggleButton`s bound to `HourToggleViewModel`s. Timed activation is a checkbox on the tab (not a Start button) - `IsTimedActivationEnabled` calls `ActivateFor` when checked and `ClearTimedActivation` when unchecked; countdown is a `DispatcherTimer`-driven string. State is a tri-state (Off / Armed / Active) surfaced as a colored bubble in the tab header, text in the tray menu, and a bottom-bar toggle. Poll cadence (5 s) is decoupled from the user-facing interval (default 45 s) so the "send if idle N seconds" guarantee is tight regardless of the user's chosen interval.
- **Add a diagnostic log point**: add a new method to `Services/DiagLog.cs` (with typed params and the format string in its body), pick or add a `LogCategory`, and call it from the relevant site. Never write log strings inline elsewhere - the point of DiagLog is single-source strings, no drift. `DiagLog.Emit` short-circuits when disabled; there is no perf cost for adding call sites in hot paths.
- **Persist a window's position**: use `Views/WindowPlacement.Restore/Snapshot`. Add `WindowX/Y/Width/Height` to `AppSettings` (nullable ints), call `Restore` in the window's `Opened`, call `Snapshot` + `AppHost.UpdateSettings` in `Closing`. `Restore` validates the saved rect lands on a currently-connected screen; falls back silently if not.
